...
Version: 1.0.0
Description: Function definitions for Hard Core Perturbation Theory calculation.
"""
...
# hcpt modules.
from src.data import DISTANCE, PAIR_DISTRIBUTION
from src.data import RESOLUTION_SIGMA, RHO
...
def delta_exact(b, q):
	...
	return 0.5 * q * (0.5 * b * np.pi - local_r0 + local_int)
...
def gamma_asymp(x, R):
	return 2.0*np.pi*(0.0 + 1.0j)*RHO*quad_no_warnings(integrand_asymp,
                                                           0, R, args = (x))[0]
